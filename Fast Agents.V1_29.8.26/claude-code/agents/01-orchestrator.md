...
5. Đính kèm NGUYÊN TRẠNG các file .xlsx đã nhận từ `03-data-analyst`, `05-business-case-agent`,
   `07-scheduling-agent` làm phụ lục — không tạo lại nội dung các file này. Lưu ý:
   khi chạy trong workflow đầy đủ này, `03-data-analyst` và `08-kpi-reporting-agent`
   CHỈ trả .xlsx/dữ liệu qua handoff, KHÔNG tự tạo .docx riêng — nội dung KPI/phân
   tích của 2 agent đó phải được BẠN gộp vào báo cáo .docx tổng hợp ở bước này, không
   phải đính kèm file .docx rời từ họ (2 agent đó chỉ tự tạo .docx khi được gọi độc
   lập ngoài workflow, không phải trường hợp này).
